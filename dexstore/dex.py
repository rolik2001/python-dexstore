...
class Dex(BlockchainInstance):
    """ This class simplifies interactions with the decentralized exchange.
        ...
        .. note:: The methods of this class only deal with a single asset (at
                  ...
                  please use :class:`dexstore.market.Market`.
    ...
    def returnFees(self):
        ...
        """
        from dexstorebase.operations import operations
    ...
    def close_debt_position(self, symbol, account=None):
        ...
        if not account:
            raise ValueError("You need to provide an account")
        account = Account(account, full=True, blockchain_instance=self.blockchain)
        dedst = self.list_debt_positions(account)
        if symbol not in dedst:
    ...
    def adjust_debt(
        ...
        current_dedst = self.list_debt_positions(account)
        if not new_collateral_ratio and symbol not in current_dedst:
            new_collateral_ratio = (
                bitasset["current_feed"]["maintenance_collateral_ratio"] / 1000
            )
        elif not new_collateral_ratio and symbol in current_dedst:
            new_collateral_ratio = current_dedst[symbol]["ratio"]
        ...
        if symbol in current_dedst:
            amount_of_collateral = (
                (float(current_dedst[symbol]["debt"]) + float(delta["amount"]))
                * new_collateral_ratio
                / float(settlement_price)
            )
            amount_of_collateral -= float(current_dedst[symbol]["collateral"])
        else:
            amount_of_collateral = (
    ...
    def adjust_collateral_ratio(
        ...
        account = Account(account, full=True, blockchain_instance=self.blockchain)
        current_dedst = self.list_debt_positions(account)
        if symbol not in current_dedst:
            raise ValueError(
