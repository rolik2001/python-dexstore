...
class Market(dict):
    """ This class allows to easily access Markets on the blockchain for trading, etc.
        ...
        * ``base`` and ``quote`` are valid assets (according to :class:`dexstore.asset.Asset`)
        * ``base:quote`` separated with ``:``
        * ``base/quote`` separated with ``/``
        * ``base-quote`` separated with ``-``

        .. note:: Throughout this library, the ``quote`` symbol will be
                  presented first (e.g. ``USD:DST`` with ``USD`` being the
                  quote), while the ``base`` only refers to a secondary asset
                  for a trade. This means, if you call
                  :func:`dexstore.market.Market.sell` or
                  :func:`dexstore.market.Market.buy`, you will sell/buy **only
    ...
    def get_string(self, separator=":"):
        """ Return a formated string that identifies the market, e.g. ``USD:DST``
    ...
    def ticker(self):
        """ Returns the ticker for all markets.
            ...
            * ``core_exchange_rate``: Core exchange rate for payment of fee in non-DST asset
    ...
    def orderbook(self, limit=25):
        """ Returns the order book for a given market. You may also
            ...
            .. code-block:: js
                ...
                {'bids': [0.003679 USD/DST (1.9103 USD|519.29602 DST),
                0.003676 USD/DST (299.9997 USD|81606.16394 DST),
                0.003665 USD/DST (288.4618 USD|78706.21881 DST),
                0.003665 USD/DST (3.5285 USD|962.74409 DST),
                0.003665 USD/DST (72.5474 USD|19794.41299 DST)],
                'asks': [0.003738 USD/DST (36.4715 USD|9756.17339 DST),
                0.003738 USD/DST (18.6915 USD|5000.00000 DST),
                0.003742 USD/DST (182.6881 USD|48820.22081 DST),
                0.003772 USD/DST (4.5200 USD|1198.14798 DST),
                0.003799 USD/DST (148.4975 USD|39086.59741 DST)]}


            .. note:: Each bid is an instance of
                class:`dexstore.price.Order` and thus carries the keys
    ...
    def accountopenorders(self, account=None):
        """ Returns open Orders
            ...
            :param dexstore.account.Account account: Account name or instance of Account to show orders for in this market
        """
        if not account:
            if "default_account" in self.blockchain.config:
                account = self.blockchain.config["default_account"]
    ...
    def buy(
        ...
        **kwargs
        ...
        """ Places a buy order in a given market
            ...
            Prices/Rates are denoted in 'base', i.e. the USD_DST market
            is priced in DST per USD.

            **Example:** in the USD_DST market, a price of 300 means
            a USD is worth 300 DST

            .. note::

                All prices returned are in the **reversed** orientation as the
                market. I.e. in the BTC/DST market, prices are DST per BTC.
            ...
            .. warning::
                ...
                for. Example:
                    ...
                    * You place and order to buy 10 USD for 100 DST/USD
                    * This means that you actually place a sell order for 1000 DST in order to obtain **at least** 10 USD
