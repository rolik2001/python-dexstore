.. python-dexstore documentation master file, created by
...
.. image:: _static/python-dexstore-logo.svg
   :width: 300 px
   :alt: Python DexStore
   :align: center

Welcome to pydexstore's documentation!
=======================================

DexStore is a **blockchain-based autonomous company** (i.e. a DAC) that
...
solution. In case of DexStore, it comes with decentralized trading of
assets as well as customized on-chain smart contracts.

About this Library
------------------

The purpose of *pydexstore* is to simplify development of products and
services that use the DexStore blockchain. It comes with
...
.. code-block:: python
   ...
   dexstore = DexStore()
   dexstore.wallet.create("secret-passphrase")
   dexstore.wallet.addPrivateKey("<wif-key>")
...
.. code-block:: python
   ...
   from dexstore import DexStore
   dexstore = DexStore()
   dexstore.wallet.unlock("wallet-passphrase")
   dexstore.transfer("<to>", "<amount>", "<asset>", "[<memo>]", account="<from>")

Monitor the DexStore Blockchain operation-wise
...
.. code-block:: python
   ...
   print(market.ticker())
   market.dexstore.wallet.unlock("wallet-passphrase")
...
.. code-block:: python
   ...
   from dexstore.dex import Dex
   dex = Dex()
   dex.dexstore.wallet.unlock("wallet-passphrase")
...
a [dedicated Telegram Channel](https://t.me/pydexstore)
